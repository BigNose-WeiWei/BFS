...
public class World
{
    ...
    {
        UsingColor(ConsoleColor.Magenta, ConsoleColor.Black, () =>
        {
            for (int row = 0; row < DotsString.Length; row++)
            {
                for (int col = 0; col < DotsString[0].Length; col++)
    ...
    {
        pacman.PowerTimes--;
        if (pacman.PowerTimes == 0) ghosts.Members.ForEach(x => x.Weak = false);
    }
    ...
    {
        var isGameOver = false;
        var meetGhosts = ghosts.Members
            .Where(ghost => ghost.Position == pacman.Position)
            .ToArray();

        if(meetGhosts.Length == 0) return false;

        foreach (var ghost in meetGhosts)
        {
            if (ghost.Weak)
            {
                _score += 10;
    ...
    {
        if (HasMet(pacman, ghosts)) 
            return Fight(pacman, ghosts);
        return false;
    }
    ...
    public Direction GhostsAlgorithmDirection(Ghost ghost, Pacman pacman)
    {
        ...
        return GetDirction(ghost.Position, nextPosition);
    }

    private Direction GetDirction(Position start, Position end)
    {
        var row = start.Row;
